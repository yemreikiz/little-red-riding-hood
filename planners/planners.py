...
class Node:
    ...
    def draw(self, surf, node_radius, width):
        ...
        pygame.draw.rect(surf, Color.LIGHTGREY, rect, width=2)
...
class GridBasedSearch:
    ...
    def sample(self, sample_size):
        ...
        for i in range(10, int(self.mapx - 10), 20):
            for j in range(10, int(self.mapy - 10), 20):
                n = len(self.nodes)
                self.add_node(n, i, j)
        """
    ...
    def sample_envir(self):
        x = int(random.randrange(10, self.mapx, 20))
        y = int(random.randrange(10, self.mapy, 20))
    ...
    def add_connectivity(self, n):
        neighbors = {}
        if n == 4:
            for node in self.nodes:
                for x in range(node.x - 10, node.y + 30, 20):
                    for y in range(node.y - 10, node.y + 30, 20):
                        neighbors[x + y] =
    ...
    def create_graph(self, surf, nr, neighbors):
        self.k = neighbors
        for node in self.nodes:
            node.adj = {}
            node.edge = {}
            self.add_connectivity(node)
            node.draw(surf, nr, 1)

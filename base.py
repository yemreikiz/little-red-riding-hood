...
from planners.planners import GridBasedSearch, Color, RRT, CircularObstacle
...
def sample_envir(map_pos, map_dim, obs_dim):
    ...
    x = int(random.randrange(sx, ex, 20))
    y = int(random.randrange(sy, ey, 20))
    return (x, y)
...
class App:
    def __init__(self):
        ...
        self.size = self.width, self.height = int(1600 * self.sc), int(840 * self.sc) # og: 1600 x 820, test: 2000 x 900
        ...
        self.planners = ['Grid Based Search', "RRT"]
        self.default_planner = 'Grid Based Search'
        self.state = State.GBS
    ...
    def on_init(self):
        pygame.init()
        pygame.display.set_caption("Little Red Riding Hood Project")
        ...
        self.option_ui_windows = {
            ...
            State.GBS: pygame_gui.core.UIContainer(
                relative_rect=pygame.Rect((0, 120), self.optionp_size), manager=self.manager,
                object_id='GBS', container=self.option_ui_panel),
        }
        ...
        self.GBS_ui_options = {
            'title': pygame_gui.elements.UILabel(relative_rect=pygame.Rect(20, 20, 121, 40),
                                                 text='GBS Options',  
                                                 manager=self.manager,
                                                 container=self.option_ui_windows[State.GBS],
                                                 object_id='header'),
            'sample_slider_textbox': pygame_gui.elements.UILabel(relative_rect=pygame.Rect(20, 60, 145, 35), 
                                                                 text=f'Sample Size: {self.GBS_options["sample_size"]}',
                                                                 manager=self.manager,
                                                                 container=self.option_ui_windows[State.GBS]),
            'sample_slider': pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect(20, 95, 250, 40),
                                                                    start_value=self.GBS_options['sample_size'], 
                                                                    value_range=(100, 10000),
                                                                    manager=self.manager,
                                                                    container=self.option_ui_windows[State.GBS]),
            'neighbour_slider_textbox': pygame_gui.elements.UILabel(relative_rect=pygame.Rect(20, 150, 196, 35),
                                                                    text=f'Connect to {self.GBS_options["neighbours"]} neighbours', 
                                                                    manager=self.manager, container=self.option_ui_windows[State.GBS]),
            'neighbour_slider': pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect(20, 185, 250, 40),
                                                                       start_value=self.GBS_options['neighbours'], 
                                                                       value_range=(1, 20),
                                                                       manager=self.manager,
                                                                       container=self.option_ui_windows[State.GBS]),
            'set_k': pygame_gui.elements.UIButton(relative_rect=pygame.Rect(20,235,250,40), text='Set Neighbours', manager=self.manager, container=self.option_ui_windows[State.GBS]),
    ...
    def init_state(self):
        if self.state == State.GBS:
            self.planner = GridBasedSearch(self.map_size, self.start_pose, self.start_radius, self.goal_pose,
                                                self.goal_radius, self.obstacles,
                                                self.GBS_options['neighbours'])
            self.search = AStar(self.planner.nodes)

            self.t = threading.Thread(target=self.planner.sample, args=(self.GBS_options['sample_size'],))
            self.t.start()
    ...
    def on_ui_event(self, event):
        if event.type == pygame.USEREVENT:
            if event.user_type == pygame_gui.UI_DROP_DOWN_MENU_CHANGED:
                ...
                if event.ui_element == self.GBS_ui_options['search_options']:
            ...
            if event.user_type == pygame_gui.UI_HORIZONTAL_SLIDER_MOVED:
                ...
                if event.ui_element == self.GBS_ui_options['sample_slider']:
                    self.GBS_options['sample_size'] = event.value
                    self.GBS_ui_options['sample_slider_textbox'].kill()
                    self.GBS_ui_options['sample_slider_textbox'] = pygame_gui.elements.UILabel(relative_rect=pygame.Rect(20, 60, 145, 35),
                        text=f'Sample Size: {self.GBS_options["sample_size"]}', 
                        manager=self.manager,
                        container=self.option_ui_windows[State.GBS])
                    self.update_GBS_samples()

                if event.ui_element == self.GBS_ui_options['neighbour_slider']:
                    self.GBS_options['neighbours'] = event.value
                    self.GBS_ui_options['neighbour_slider_textbox'].kill()
                    self.GBS_ui_options['neighbour_slider_textbox'] = pygame_gui.elements.UILabel(relative_rect=pygame.Rect(20, 150, 196, 35),
                        text=f'Connect to {self.GBS_options["neighbours"]} neighbours', 
                        manager=self.manager,
                        container=self.option_ui_windows[State.GBS])
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                ...
                if event.ui_element == self.visualize_button:
                    self.simulateState()
                if event.ui_element == self.GBS_ui_options['set_k']:
                    self.planner.update_k(self.GBS_options['neighbours'])
                    self.search.path = []
    ...
    def simulateState(self):
        if self.state == State.GBS:
            self.planner.create_network(self.map, self.node_radius, self.GBS_options['neighbours'])
